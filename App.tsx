...
const App: React.FC = () => {
  ...
  // Sync to localStorage whenever history or favorites change (only after initial load)
  useEffect(() => {
    if (isInitialized) {
  ...
  const handleSpark = async () => {
    ...
    try {
      ...
      alert(
        "The spark flickered out... please check your connection and try again.",
      );
    } finally {
      setIsLoading(false);
    }
  ...
  const isBookmarked = currentIdea
    ? favorites.some((f) => f.id === currentIdea.id)
    : false;

  return (
    <div
      className={`min-h-screen relative flex flex-col items-center p-4 md:p-8 overflow-x-hidden ${darkMode ? "bg-stone-900 text-stone-100" : ""}`}
    >
      {/* Dynamic Background */}
      <div className="fixed inset-0 pointer-events-none overflow-hidden -z-10">
        <div
          className={`absolute -top-[10%] -left-[10%] w-[40%] h-[40%] rounded-full blur-[120px] opacity-40 ${darkMode ? "bg-rose-900" : "bg-rose-200"}`}
        ></div>
        <div
          className={`absolute -bottom-[10%] -right-[10%] w-[40%] h-[40%] rounded-full blur-[120px] opacity-40 ${darkMode ? "bg-red-900" : "bg-red-100"}`}
        ></div>
      </div>
      ...
      <header className="w-full max-w-4xl flex justify-between items-center mb-12">
        ...
        >
          ...
          <h1
            className={`text-2xl font-bold tracking-tight ${darkMode ? "text-stone-100" : "text-stone-800"}`}
          >
            Spark
          </h1>
        </button>
        ...
        <div className="flex items-center gap-3">
          ...
          >
            ...
            <span
              className={`text-xs px-2 py-0.5 rounded-full font-bold ${darkMode ? "bg-rose-900 text-rose-300" : "bg-rose-100 text-rose-700"}`}
            >
      ...
      <main className="w-full max-w-2xl flex flex-col items-center">
        {!currentIdea && !isLoading && (
          <div className="text-center py-16 md:py-24 space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-1000">
            <h2
              className={`text-4xl md:text-6xl font-serif leading-tight ${darkMode ? "text-stone-100" : "text-stone-900"}`}
            >
              Practical ideas.
              <br />
              Real roadmaps.
              <br />
              <span className="text-[#FF91A4] italic">Built for builders.</span>
            </h2>
            <p
              className={`text-lg max-w-md mx-auto leading-relaxed ${darkMode ? "text-stone-400" : "text-stone-500"}`}
            >
              Skip the fluff. Get a grounded, actionable project idea tailored
              to your stack and timeline.
            </p>
          </div>
        )}
        ...
        {isLoading && (
          <div className="flex flex-col items-center justify-center py-24 space-y-6">
            ...
            <p
              className={`font-medium animate-pulse text-xl ${darkMode ? "text-stone-400" : "text-stone-500"}`}
            >
              Engineering your next build...
            </p>
          </div>
        )}
      ...
      <footer
        className={`mt-auto pt-16 pb-8 text-sm flex items-center gap-8 ${darkMode ? "text-stone-500" : "text-stone-400"}`}
      >
